...
def compute(api_server: str, headers: dict, oid: int, mtype: str='part', verbose: bool=False, debug: bool=False):
    ...
    print(f"hoop_stress.compute: api_server={api_server}, mtype={mtype}, otype='site', id={oid}, name={part['name']}")
    cwd = os.getcwd()
    print(f'cwd={cwd}')
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        for i in range(nsites): # track(range(nsites), description=f"Processing sites for part {part['name']}..."):
            ...
            for magnet in site['site_magnets']:
                ...
                for part in _object['magnet_parts']:
                    ...
                    if _ptype in ['helix', 'bitter', 'supra']:
                        _pobject = utils.getobject(f"{api_server}", headers=headers, mtype='part', id=_pid, verbose=verbose, debug=debug)
            ...
            # get record data
            total = 0
            # records = utils.gethistory(api_server, headers, site['site_id'], mtype='site', otype='record', verbose=debug, debug=debug) #site_obj['records']
            nrecords = len(site['records'])
            for j in track(range(nrecords), description=f"Processing records for site id={site['name']}..."):
                f = site['records'][j]
                # print(f'f={f}')
                ...
                for _ih, _ib in zip(df['IH_ref'].to_numpy(), df['IB_ref'].to_numpy()):
                    ...
                    vcurrents = list(icurrents)
                    num = 0
                    if len(Tubes) != 0: vcurrents[num] = _ih; num += 1
                    if len(BMagnets) != 0: vcurrents[num] = _ib; num += 1
                    if len(UMagnets) != 0: vcurrents[num] = 0; num += 1
